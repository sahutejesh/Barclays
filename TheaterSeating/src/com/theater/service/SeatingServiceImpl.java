...
import com.theater.command.TheaterLayout;
import com.theater.command.TicketRequest;
import com.theater.command.TheaterSection;
...
public class SeatingServiceImpl implements SeatingService {
	 ...
	 */
	private int findSectionByAvailableSeats(List<TheaterSection> sections,
			int availableSeats) {
	...
	public void processTicketRequests(TheaterLayout layout,
				...
				if (request.getNoOfTickets() == section.getAvailableSeats()) {
					...
					if (requestNo != -1) {
						...
						if (sectionNo >= 0) {
							...
							TheaterSection perferctSection = sections
									.get(sectionNo);
